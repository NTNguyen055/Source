...
if (isset($_GET['remove'])) {
    ...
    $delete = mysqli_query($conn, "DELETE FROM `user_form` WHERE id = '$remove_id'") or die('query failed');
...
if (!empty($sxField) && !empty($sxSort)) {
    $sxCondition = "ORDER BY `user_form`.`" . $sxField . "` " . $sxSort;
...
if ($search) {
    $selectMain = mysqli_query($conn, "SELECT * FROM `user_form` WHERE `name` LIKE '%" . $search . "%' " . $sxCondition . " LIMIT " . $per_page . " OFFSET " . $offset) or die('query failed');
    $totalPro = mysqli_query($conn, "SELECT * FROM `user_form` WHERE `name` LIKE '%" . $search . "%'");
} else {
    $selectMain = mysqli_query($conn, "SELECT * FROM `user_form` " . $sxCondition . " LIMIT " . $per_page . " OFFSET " . $offset) or die('query failed');
    $totalPro = mysqli_query($conn, "SELECT * FROM `user_form`");
}
            ...
            <div class="content-main">
                <div class="content-box">
                    <div class="content-box-title">
                        ...
                        <a href="adminAcc.php" title="Ấn vào để sang trang quản trị tài khoản!">Danh sách người quản trị</a>
                    </div>
                            ...
                            ?>
                                    <div class="content-box-pro-main">
                                        ...
                                        <div class="content-box-pro-6">
                                            <div class="cbp-content">
                                                <a href="user.php?remove=<?php echo $fetch['id']; ?>" onclick="return confirm('Bạn muốn xóa tài khoản này?')">Xóa</a>
                                            </div>
                                        </div>
                                    </div>
                            <?php
